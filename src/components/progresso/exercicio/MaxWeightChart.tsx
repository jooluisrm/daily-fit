...
type ChartData = { date: string, maxWeight: number }[];

export function MaxWeightChart({ data, weightType = "TOTAL" }: { data: ChartData, weightType?: string }) {
  ...
  return (
    <div className="bg-gradient-to-br from-zinc-900/90 to-zinc-950 border border-zinc-800/50 p-6 rounded-2xl flex flex-col h-[350px] relative overflow-hidden group hover:border-zinc-700/50 transition-all shadow-sm">
      ...
      <div className="mb-6 relative z-10">
        <h3 className="text-lg font-bold text-white">Carga Máxima (Pico)</h3>
        <p className="text-sm text-zinc-400">Maior peso levantado em qualquer série {labelSuffix}</p>
      </div>
      <div className="flex-1">
        <ChartContainer config={{ maxWeight: { label: `Max ${labelSuffix}`, color: "#8b5cf6" } }} className="h-full w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="maxWeight" stroke="#8b5cf6" strokeWidth={3} dot={{ r: 4, fill: "#8b5cf6", strokeWidth: 0 }} />
            </LineChart>
          </ResponsiveContainer>
        </ChartContainer>
      </div>
